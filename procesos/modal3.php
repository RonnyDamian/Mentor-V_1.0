            ...
            <div class="modal-body">
                ...
                <div class="progress">
                    <div class="progress-bar progress-bar-striped active" role="progressbar"
                         aria-valuenow="20" aria-valuemin="20" aria-valuemax="20" style="width:40%;">
                        40%
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" data-dismiss="modal" id="continue">Continuar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $( document ).ready(function() {
        ...
        setInterval(function(){
                while(progressNumber<=60){
                    progressBar.css('width', progressNumber + '%');
                    progressBar.attr('aria-valuenow', progressNumber);
                    progressNumber++;
                }
            $('#messagge').html("¡Ahora falta menos! ¡Ya estamos cerca! 😎");
                $(".progress-bar").html('60%');
        },1000);
    });

    $('#continue').on('click', function () {
        setTimeout("location.href='questions4.php' ", 100);
    })
</script>
<script>
    $('#myModal').modal({
